...
class testVerifyEmptyDatabase(unittest.TestCase):
    def setUp(self):
        ...
        self.meet1 = start_tms_and_get_meet(EXAMPLE_MEETS[0])
...
def start_tms_and_get_meet(meet_info_dict):
    unattached = School.init_unattached_school()
    ...
    meet = Meet(name=meet_info_dict['name'],
                ...
                status=meet_info_dict.get('status', 'accepting_entries'))
    db.session.add(meet)
    db.session.commit()
